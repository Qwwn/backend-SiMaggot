...
class SellerServices {
  ...
  async addSeller (data) {
    try {
      const { name: shopName, sellerId } = data
      const isSellerNameExist = await this.verifySellerExistByName(shopName)
      if (isSellerNameExist) {
        throw new ClientError('Shop name already exists')
      }
      ...
      const doc = db.collection('seller').doc(shopName)
      await doc.set(data)
      return sellerId
    } catch (error) {
      throw error
